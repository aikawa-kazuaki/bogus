...
// ----------------------------------------------------------------------
// 優先度ソート機能（未出題 ＞ 正解率が低い順 ＞ その他 / グループ内はランダム）
// ----------------------------------------------------------------------
function prioritizeQuestions(array) {
    const unhandled = []; // 未出題
    const wrong = [];     // 解いたことがあり、不正解率50%以上
    const lowAccuracy = [];// 解いたことがあり、正解率50%〜99%
    const perfect = [];   // 解いたことがあり、正解率100%
...
// ----------------------------------------------------------------------
// 履歴の手動エクスポート / インポート機能 (Mac ⇔ iPhone 同期用)
// ----------------------------------------------------------------------
...
function exportData() {
    ...
    downloadAnchor.setAttribute("download", `gas_quiz_backup_${dateStr}.json`);
...
function importData(event) {
    ...
    reader.onload = function(e) {
        try {
            ...
            if (!importedData || (!importedData.detailedHistory && !importedData.bookmarks)) {
                alert("無効なデータファイルです。");
                return;
            }
            ...
            if (importedData.detailedHistory) {
                Object.keys(importedData.detailedHistory).forEach(key => {
                    if (!detailedHistory[key]) {
                        detailedHistory[key] = importedData.detailedHistory[key];
                    } else {
            ...
            if (importedData.bookmarks && Array.isArray(importedData.bookmarks)) {
                const combinedBookmarks = new Set([...bookmarks, ...importedData.bookmarks]);
                bookmarks = Array.from(combinedBookmarks);
                localStorage.setItem('gas_quiz_bookmarks', JSON.stringify(bookmarks));
            }

            updateStatsSummary();
            alert("学習履歴およびブックマークを正常にインポートしました！");
            location.reload();
        } catch (err) {
            console.error(err);
